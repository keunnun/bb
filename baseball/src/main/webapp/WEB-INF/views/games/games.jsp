...
<%@page import="java.time.LocalDate"%>
	...
	<table width="1050" align="center">
		<caption>
			<a href="../games/games?y=<%=y - 1%>&m=<%=m%>"> &lt&lt </a>
			<%
				if (m == 1) {
			%>
			<a href="../games/games?y=<%=y - 1%>&m=12"> <b>&lt</b> </a>
			<%
				} else {
			%>
			<a href="../games/games?y=<%=y%>&m=<%=m - 1%>"> <b>&lt</b> </a>
			<%
				}
			%>
			<select name="y" onchange="location.href=this.value">
			<%for (int i=2012;i<=2022;i++){ %>
				<option value="../games/games?y=<%=i%>&m=<%=m%>" 
				<%if (i==y) {%>
				selected<%} %>
				><%=i%> 년</option>
			<%} %>
			...
			<%for (int j=1;j<13;j++){ %>
				<option value="../games/games?y=<%=y%>&m=<%=j%>"
				<%if (j==m) {%>
				selected<%} %>><%=j %> 월</option>
			<%} %>
			</select>
			...
			<%
				if (m == 12) {
			%>
			<a href="../games/games?y=<%=y + 1%>&m=1"> <b>&gt</b> </a>
			<%
				} else {
			%>
			<a href="../games/games?y=<%=y%>&m=<%=m + 1%>"> <b>&gt</b> </a>
			<%
				}
			%>
			<a href="../games/games?y=<%=y + 1%>&m=<%=m%>"> &gt&gt </a>
			<hr>
			<a href="../games/list?y=<%=y%>&m=<%=m%>"">Go to list</a>
			<br><br>
		</caption>
		...
		<tr>
			...
			%>
			<hxt><%=dl.get(k).getHxt()%></hxt>  <sc><%=dl.get(k).getHxscore()%> : <%=dl.get(k).getExscore()%></sc>  <ext><%=dl.get(k).getExt()%></ext><br>
			<%}%>
			</td><%	day++;
						}
				}
